...
iftImage *myDelineateEggComp(
  iftImage *comps, iftSet *SelectedComps, iftMImage *mimg,
  float seed_erosion, float seed_dilation, int arc_id
)
{
  /* Set inner markers */
  iftImage *seeds_in = iftCreateImage(comps->xsize, comps->ysize, comps->zsize);
  ...
  iftLabeledSet *LS = NULL;
  for (int p = 0; p < mimg->n; p++) {
      if (seeds_in->val[p] != 0) {
          // object/internal seed; you may keep seeds_in->val[p] if multi-label
          iftInsertLabeledSet(&LS, p, seeds_in->val[p]); 
      } else if (seeds_out->val[p] != 0) {
          // external/background seed, label 0
          iftInsertLabeledSet(&LS, p, 0);
      }
  }

  iftImage *label = myDynamicTree(mimg, LS, arc_id);
  iftDestroyImage(&seeds_in);
  iftDestroyImage(&seeds_out);
...
iftImage *mySMansoniDelineation(
  iftMImage *mimg, iftImage *saliency, int border_dist,
  int min_obj_area, int max_obj_area, float seed_erosion,
  float seed_dilation, int arc_id
)
{
  iftImage *aux1 = NULL, *aux2 = NULL;
  int Lmax=0;
  ...
  if ((Lmax=iftMaximumValue(aux1)) > 0) {
    ...
    for (int j=1; j <= Lmax; j++)
      iftInsertSet(&SelectedComps, j);
    // Delineate components
    aux2 = myDelineateEggComp(aux1, SelectedComps, mimg, \
			       seed_erosion, seed_dilation, arc_id);
    /* Select components within a given area interval */
    iftDestroyImage(&aux1);
    aux1 = iftSelectCompInAreaInterval(aux2, NULL, \
				       min_obj_area, max_obj_area);
